...
class Analysis(models.Model):
    ...
    claim = models.OneToOneField(
        Claim, on_delete=models.CASCADE, primary_key=True, related_name="analysis"
    )
    ...
    model_prediction = models.CharField(
        max_length=20, choices=ModelPrediction.choices, null=True, blank=True
    )
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(
                check=models.Q(credibility_score__gte=0)
                & models.Q(credibility_score__lte=100),
                name="valid_credibility_score_range",
            ),
            models.CheckConstraint(
                check=models.Q(final_weighted_score__gte=0)
                & models.Q(final_weighted_score__lte=100),
