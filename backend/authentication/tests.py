...
from authentication.exceptions import AccountDisabledError, InvalidCredentialsError
...
class RefreshTokenAPITests(TestCase):
    def setUp(self):
        ...
        self.valid_payload = {"refresh_token": str(self.refresh)}
    ...
    def test_refresh_token_success(self):
        ...
        self.assertNotEqual(
            inner_data["refresh_token"], self.valid_payload["refresh_token"]
        )
    ...
    def test_new_refresh_token_works(self):
        ...
        response2 = self.client.post(
            self.url, data=json.dumps(payload2), content_type="application/json"
        )
...
class RBACIntegrationTests(TestCase):
    ...
    def test_rbac_allows_user_and_admin(self):
        ...
        user_token = self._get_access_token(
            "user_role@example.com", "StrongPassword123!"
        )
        admin_token = self._get_access_token(
            "admin_role@example.com", "StrongPassword123!"
        )
    ...
    def test_rbac_rejects_user_from_admin_only_requirement(self):
        ...
        self.assertEqual(
            str(context.exception), "You do not have permission to perform this action."
        )
