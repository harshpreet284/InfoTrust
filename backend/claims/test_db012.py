...
class DB012ValidationTests(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(email='test@example.com', password='pwd', full_name='Test User')
        
    def test_valid_claim_text_accepted(self):
        """1. Valid Claim text is accepted."""
        claim = Claim(user=self.user, text="This is a perfectly valid claim with more than 10 characters.")
    ...
    def test_valid_credibility_score_accepted(self):
        ...
        analysis = Analysis(claim=claim, credibility_score=50.0, final_weighted_score=50.0)
        analysis.full_clean()
        analysis.save()
    ...
    def test_valid_final_weighted_score_accepted(self):
        ...
        claim = Claim.objects.create(user=self.user, text="This is a valid claim.")
        analysis = Analysis(claim=claim, credibility_score=100.0, final_weighted_score=0.0)
        analysis.full_clean()
        analysis.save()
        self.assertIsNotNone(analysis.pk)
    ...
    def test_required_fields_behave_correctly(self):
        ...
        claim = Claim(user=self.user, text="")  # empty string should fail required validation (or min length)
        with self.assertRaises(ValidationError) as ctx:
            claim.full_clean()
        self.assertTrue('text' in ctx.exception.message_dict)
