...
class DB011IndexTests(TestCase):
    ...
    def test_claim_submitted_at_indexed(self):
        ...
        with connection.cursor() as cursor:
            ...
            self.assertTrue(
                submitted_at_indexed, "Claim submitted_at should be indexed"
            )
    ...
    def test_claim_user_indexed(self):
        ...
        with connection.cursor() as cursor:
            ...
            user_indexed = any(
                c["columns"] == ["user_id"] and c["index"] for c in constraints.values()
            )
    ...
    def test_analysis_verdict_indexed(self):
        ...
        with connection.cursor() as cursor:
            constraints = connection.introspection.get_constraints(
                cursor, "analysis_analysis"
            )

            verdict_indexed = any(
                c["columns"] == ["verdict"] and c["index"] for c in constraints.values()
            )
    ...
    def test_analysis_credibility_score_indexed(self):
        ...
        with connection.cursor() as cursor:
            ...
            self.assertTrue(
                score_indexed, "Analysis credibility_score should be indexed"
            )
