...
class DB009RelationshipTests(TestCase):
    def setUp(self):
        self.user1 = User.objects.create_user(email='user1@example.com', password='Password123!', full_name='User One')
        self.user2 = User.objects.create_user(email='user2@example.com', password='Password123!', full_name='User Two')
    ...
    def test_claim_feedback_relationship(self):
        ...
        feedback1 = ClaimFeedback.objects.create(user=self.user1, claim=claim, feedback_type=FeedbackType.HELPFUL)
        feedback2 = ClaimFeedback.objects.create(user=self.user2, claim=claim, feedback_type=FeedbackType.NOT_HELPFUL)
    ...
    def test_user_feedback_relationship(self):
        ...
        feedback1 = ClaimFeedback.objects.create(user=self.user2, claim=claim1, feedback_type=FeedbackType.HELPFUL)
        feedback2 = ClaimFeedback.objects.create(user=self.user2, claim=claim2, feedback_type=FeedbackType.HELPFUL)
    ...
    def test_user_audit_logs_relationship(self):
        ...
        log2 = AuditLog.objects.create(user=self.user1, action='VIEW_CLAIM', target='Claim 1')
    ...
    def test_cascade_and_set_null_behavior(self):
        ...
        user = User.objects.create_user(email='delete_me@example.com', password='Password123!', full_name='Delete Me')
        ...
        feedback = ClaimFeedback.objects.create(user=user, claim=claim, feedback_type=FeedbackType.HELPFUL)
